...
4. Исправляем класс `App\Entity\User`
    ...
    ```
5. Исправляем класс `App\Domain\Command\CreateUser\CreateUserCommand`
    ```php
    <?php
    ...
    ```
6. Перезапускаем контейнер супервизора командой `docker-compose restart supervisor`
7. Выполняем запрос Add user v5 из Postman-коллекции v9. Видим, что запись в БД создалась.
...
3. Исправляем файл `depfile.yaml`
    ```yaml
    parameters:
      paths:
        - ./src
