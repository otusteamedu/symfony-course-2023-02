...
5. Ещё раз выполняем запрос Add followers из Postman-коллекции v9 с параметром async = 1. Получаем сообщение в
...
3. Исправляем класс `App\DTO\AddFollowersDTO`
    ...
    ```
4. Очищаем очередь в интерфейсе RabbitMQ
5. Выходим из контейнера и запускаем supervisor командой `docker-compose start supervisor`
6. Ещё раз выполняем запрос Add followers из Postman-коллекции v9 с параметром async = 1. Видим, что пользователи
добавились в БД.
...
5. Пересобираем и перезапускаем контейнеры командами
    ...
    ```
6. Выполняем запрос Post tweet из Postman-коллекции v9 с параметром async = 1. Видим, что уведомления добавились в БД.
...
5. Выполняем запрос Post tweet из Postman-коллекции v9 с параметром async = 1. Видим, что лента для пользователя с id=5
...
3. В файле `config/packages/messenger.yaml`
   ...
   2. добавляем секцию `messenger.buses`
       ...
       ```
4. Добавляем класс `App\MessageHandler\SendNotification\Handler`
    ```php
    <?php
